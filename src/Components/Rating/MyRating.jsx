...
const MyRating = () => {
    ...
    const [ratings, setRatings] = useState(Array(5).fill({ value: 0, clicked: false }));
    const { user} = useContext(AuthContext);

    const handleChange = (index) => {
        if (!ratings[index].clicked) {
            const newRatings = [...ratings];
            newRatings[index] = { ...newRatings[index], value: newRatings[index].value + 1, clicked: true };
    ...
    return (
        <div>
            <div className="grid gap-4 md:grid-cols-5 w-1/2 mx-auto rating rating-md">
                {ratings.map(({ value, clicked }, index) => (
                    <Rating
                        ...
                        style={{ transform: clicked ? 'scale(1.6)' : 'scale(1.0)', transition: 'transform 0.3s ease' }}
                        readOnly={clicked}
            ...
            <div className='flex justify-center gap-4 mx-auto mt-4'>
                <button className='btn btn-primary' onClick={() => setRatings(Array(5).fill({ value: 0, clicked: false }))}>
                    Reset
                </button>
                <button className='btn btn-accent' onClick={() => { handleSubmit(totalRating) }}>Submit</button>
            </div>
            <div><p className='flex justify-center gap-4 mx-auto mt-4'>Total rating: {totalRating}/5</p></div>
        </div>
    );
};
